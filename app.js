...
document.querySelectorAll("#mode-select button[data-mode]").forEach(btn => {
  btn.onclick = () => {
    appState = "flashcard";
    document.querySelector(".flashcard-only").style.display = "inline-block"; // show Flip button
...
document.getElementById("mcq").onclick = () => {
  appState = "mcq";
  document.querySelector(".flashcard-only").style.display = "none"; // hide Flip button
  generateMCQ();
};
